export default function selectBox() {
  ...
  // 시/도 선택 박스 초기화
  const sidoSelects = document.querySelectorAll("select[name^=sido]");
  sidoSelects.forEach(function (selectElement) {
    const selsido = selectElement;
    area0.forEach(function (value) {
      const option = document.createElement("option");
      option.value = value;
      option.text = value;
      selsido.appendChild(option);
    });

    const gugunSelect = selsido.nextElementSibling;
    const defaultOption = document.createElement("option");
    defaultOption.value = "";
    defaultOption.text = "구/군 선택";
    gugunSelect.appendChild(defaultOption);
  });
  let area; // area 변수를 먼저 선언
